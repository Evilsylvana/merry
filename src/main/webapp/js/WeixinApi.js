 ...
 * 7、调起微信客户端的图片播放组件 8、关闭公众平台Web页面 9、判断当前网页是否在微信内置浏览器中打开 10、支持WeixinApi的错误监控
 * 11、发送电子邮件 12、禁止用户分享
 * 
...
(function(window) {
	...
	var _share = function(cmd, data, callbacks) {
		...
		var handler = function(theData, argv) {
			...
			if (argv
					&& (argv.shareTo == 'favorite' || argv.scene == 'favorite')) {
				if (callbacks.favorite === false) {
					WeixinJSBridge.invoke('sendAppMessage', theData,
							new Function());
				} else {
				...
				if (cmd.menu === 'general:share') {
					if (argv.shareTo === 'timeline') {
						WeixinJSBridge.invoke('shareTimeline', theData,
								progress);
				...
				.on(
						...
						function(argv) {
							...
							if (callbacks.async && callbacks.ready) {
								...
								if (WeixinApi["_wx_loadedCb_"].toString()
										.indexOf("_wx_loadedCb_") > 0) {
								...
								// 然后就绪
								if (!(argv
										&& (argv.shareTo == 'favorite' || argv.scene == 'favorite') && callbacks.favorite === false)) {
									callbacks.ready
											&& callbacks.ready(argv, data);
							...
							function(key) {
								if (typeof WeixinJSBridge[key] === 'function') {
									WeixinJSBridge[key] = function() {
										...
										return _WeixinJSBridge[key].apply(
												WeixinJSBridge, arguments);
									};
								}
							});
		};
